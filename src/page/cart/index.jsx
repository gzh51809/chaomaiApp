...
import { changeOpen, changeQty, deleteSame, remove } from '@/action/cartAction';
...
class Cart extends Component {
    ...
    handleDeleteSame(id, uid) {
        ...
        this.props.goodslist.map(it => {
            if (it.id === id && it.product.length === 0) {
    ...
    render() {
        ...
        return (
            <div className="Cart">
                <TopBar handleBack={this.handleBack.bind(this)} title='购物车' />
                                                ...
                                                item.product.map(it => {
                                                    return (
                                                        <div className="product-item flex-middle" key={it.id}>
                                                            <div className="check">
                                                                <i className={`iconfont ${it.open ? 'icon-xuanzhonggou' : 'icon-weixuan'}`}></i>
                                                            </div>
